...
Based on this we define the proper subtype relation $A <_i B$ saying that $A$ is
a proper subtype of $B$ and both are beta equivalent to some terms $A'$ and $B'$
which figure in the predecessor relation with arity $i$ and are beta equivalent
to $A$ and $B$ respectively.

By ignoring the arity we get the proper subtype relation $A < B$ and by adding
reflexivity with respect to beta equivalence we get the subtype relation $A \le
B$.
...
\begin{lemma}
    ...
    {\perp}
    $$
    \begin{proof} Proof by induction on $A \prec_i B$.
        \begin{enumerate}
        \item
            $$
            \begin{array}{l|l}
            ...
            \end{array}
            $$
            $B' \prec_0 A'$ is possible only if $B' = \Any_n$ and $A' = \Any_m$
            with $-1 \le n < m$. From the beta equivalences we get $k = m$ and
            $l = n$ which are impossible to satisfy because of $k < l$ and $n <
            m$.
        ...
        \item
            ...
            \end{array}
            $$
            Because of $D \prec_{i+1} E$ there are some $A'$, $B'$ and $C'$
            with $C' \prec_i B'$, $D = \Pi x^{A'}. C'$ and $E = \Pi x^{A'}. B'$.
            The beta equivalences require $C \betaeq C'$ and $B \betaeq B'$.

            The induction hypothesis is therefore sufficent to arrive at the
            contradiction.
        \end{enumerate}
    \end{proof}
\end{lemma}
...
\begin{definition}
    ...
    $$
    If the arity is not important we write $A < B$ to express that $A$ is a
    proper subtype of $B$. This means that some arity $i$ exists such that $A
    <_i B$ is valid.
\end{definition}
...
\begin{lemma}
    ...
    $$
    \begin{proof}
        Let's assume $A <_i B$ and $B <_j C$. By definition of the relation
        $<_k$ there exist some $A'$, $B'$, $B''$ and $C''$ such that the
        ...
        \end{array}
        $$
        Since beta equivalence is transitive we get $B' \betaeq B''$. Together
        with $A' \prec_i B'$ and $B'' \prec_j C''$ and
        lemma~\ref{PrecedenceSameArity} we infer the goal $i = j$.
    \end{proof}
\end{lemma}
