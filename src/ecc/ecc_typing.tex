...
Note that the order $<$ is a linear order, in particular it is a wellorder.
...
This order is needed to define minimal types and proof that minimal types are
unique up to beta equivalence.

In order to have unique minimal types it is crucial that for any sort $s_1$
there is a unique sort $s_2$ satisfying $s_1 < s_2$.

We can define a more general calculus of construction by just requiring that the
relation $<$ on sorts is a wellfounded relation with the additional uniqueness
condition.
