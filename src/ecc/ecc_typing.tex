...
\subsection{Sorts, Order Relation and Axioms}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

In the extended calculus of constructions we use the sorts
$$
    \Prop, \Any_0, \Any_1, \ldots
$$
with the transitively closed order relation
$$
...
Note that the order $\prec$ is a linear order, in particular it is a wellorder.
Based on this linear order of sorts there is an induced partial order $<_i$ on
terms which is defined inductively by the rules
\begin{enumerate}
    \item
        $$
        \ruleh{s_1 \prec s_2}{s_1 <_0 s_2}
        $$
    \item
        $$
        \ruleh {B_1 <_i B_2}{\Pi x^A. B_1 <_{i+1} \Pi x^A. B_2}
        $$
\end{enumerate}

This order is needed to define minimal types and prove that minimal types are
unique up to beta equivalence.

In order to have unique minimal types it is crucial that for any sort $s_1$
there is a unique minimal sort $s_2$ satisfying $s_1 \prec s_2$.

We can define a more general calculus of constructions by just requiring that the
relation $\prec$ on sorts is a wellfounded relation with the additional uniqueness
condition.

We use the definition that in a wellfounded relation all elements are
accessible. An element of the carrier is accessible if all its predecessors are
accessible. This guarantees the existence of minimal elements (i.e. elements
which have no predecessors in the relation $\prec$.
...
\begin{theorem}
    ...
    \begin{proof}
        ...
        All proofs follow the same pattern. We give here the proof for the
...
\begin{theorem}
    \emph{For the function term $f$ of an application $f a$
    there exists a minimal type of $f$ of the form $\Pi x^A. B$}.
    ...
    $$
    \begin{proof} By~\ref{thm:ExistMinimalType} there exists a minimal type $F$
        of $f$ and by the generation lamma~\ref{GenerationLemma} for application
        there exists a type of the form $\Pi x^{A_3}. B_3$ for $f$. Because of
        minimality we have
        $$
        F \le \Pi x^{A_3}.B_3
        $$

        The definition of the subtype relation $\le$ has two cases. The first
        case requires that both operands are beta equivalent and in this case
        the goal is trivial. The second case requires the existence of $i$,
        $A$, $B$ and $B_2$ such that
        $$
        \begin{array}{lll}
            F & \reducestar & \Pi x^{A}. B
            \\
            \Pi x^A. B & <_{i+1} & \Pi x^A. B_2
            \\
            \Pi x^{A_3}. B_3 & \reducestar & \Pi x^A. B_2
        \end{array}
        $$
        are valid. The first line uncovers the minimal type of the required
        form.
    \end{proof}
\end{theorem}
