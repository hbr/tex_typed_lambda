...
context $\Gamma$ which assigns a type to the free variables in the term $t$.
...
In the following we define the typing relation inductively.
...
\begin{itemize}
    ...
    \item The variable introduction rule requires a welltyped type $A$ for the
        ...
        This has the important consequence that it is impossible to construct a
        welltyped abstraction of the form $\lambda x^\Any. e$. Therefore it is
        impossible to construct a function term $f$ such that $f \Prop$ is
        welltyped.

        It is possible to compute with types but it is impossible to compute
        with sorts (or kinds in general) as arguments. This is an important
        difference to the \emph{extended calculus of constructions} which allow
        variables of type $\Any_i$ because $\Any_i$ has type $\Any_{i+1}$.
\end{itemize}
