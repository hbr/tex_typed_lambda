...
It is possible to define recursive functions whose domain is an inductively
defined set. The following function $h(n)$ computes for each even number $n \in
E$ its half value.
...
Induction and recursion are just two sides of the same coin. The construction
of each even number requires finitely many applications of the rules. A
recursive function just deconstructs one rule at a time and therefore is
guaranteed to terminate.

Note that the input domain and the output domain of the function $h$ are
different. $h$ maps even numbers (i.e. elements of $E$) to natural numbers i.e.
it has the type $E \to \Nat$.
